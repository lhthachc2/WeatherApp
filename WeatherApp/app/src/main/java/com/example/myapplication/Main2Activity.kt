...
class Main2Activity() : AppCompatActivity(), Parcelable {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arrayList.add(CityModel(query))

        wordViewModel = ViewModelProvider(this).get(WordViewModel::class.java)
        var count : Int = wordViewModel.countRow()
        if (count<10)
        {
            wordViewModel.insert(CityModel(query))
        }
        else
        {
            val modelone = wordViewModel.getone
            wordViewModel.delete(modelone)
            wordViewModel.insert(CityModel(query))
            Toast.makeText(this, "Thêm thất bại", Toast.LENGTH_LONG).show()
        }

        wordViewModel.delete(CityModel(query))
        wordViewModel.insert(CityModel(query))
